...
- **AdMob의 "최대 광고 콘텐츠 등급"** — 5항 마지막 문단과 7항이 이 앱의 등급을
  **"청소년 이용가(T)"로 제한하고 있다**고 단언합니다. 이건 콘솔 설정과 짝인
  문장입니다. **AdMob → 앱 → 앱 설정 → 광고 콘텐츠 등급**에서 실제로 **T**로
  맞춰져 있어야 하고, 앱을 새로 추가할 때마다 같은 설정을 해야 합니다.

  | 등급 | 문서의 서술 |
  |---|---|
  | G · PG · T | 게재될 수 있음 (7항이 "아동에게 부적합할 수 있다"고 밝힘) |
  | **MA** | **게재되지 않는다고 단언** — 주류·담배, 사행성, 성인용 콘텐츠 |

  > 등급을 **MA로 올리면 두 곳의 문장이 곧바로 거짓이 됩니다.** 올릴 거라면
  > 문장을 먼저 지우고 시행일을 올리세요. G·PG로 **내리는** 것은 문서를
  > 어기지 않지만(상한만 낮아짐), 그때는 "MA는 게재되지 않는다"보다 더
  > 강하게 쓸 수 있으니 문장을 다듬는 편이 낫습니다.
